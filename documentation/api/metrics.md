...
> **NOTE:** This route collects metrics only from a single process. If there are no processes or more than one process, the endpoint will not return information.
...
The metrics providers and counter names to return from this route can be specified via configuration. See [TBD]() for more information.