        ...
        IPostConfigureOptions<CollectionRuleOptions>
    {
        ...
        public void PostConfigure(string name, CollectionRuleOptions options)
        {
            if (null != options.Trigger)
            {
                ...
                {
                    if (0 == ((AspNetResponseStatusOptions)options.Trigger.Settings).ResponseCount && _defaultOptions.CurrentValue.ResponseCount.HasValue)
                    {
                        ((AspNetResponseStatusOptions)options.Trigger.Settings).ResponseCount = _defaultOptions.CurrentValue.ResponseCount.Value;
                    }
                }
            }
        }
