...
public:
    ...
    HRESULT Start(const std::string& path, std::function<HRESULT (const IpcMessage& message)> callback, std::function<HRESULT (const IpcMessage& message)> validateMessage);
    void Shutdown();
...
private:
    ...
    std::function<HRESULT(const IpcMessage& message)> _validateMessage;
