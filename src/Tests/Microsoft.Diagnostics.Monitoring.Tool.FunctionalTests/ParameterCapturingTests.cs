...
namespace Microsoft.Diagnostics.Monitoring.Tool.FunctionalTests
{
    ...
    public class ParameterCapturingTests : IDisposable
    {
        ...
        public Task CapturesParametersInNonAspNetApps(Architecture targetArchitecture) =>
            CapturesParametersCore(
                TestAppScenarios.ParameterCapturing.SubScenarios.NonAspNetApp,
                targetArchitecture, CapturedParameterFormat.JsonSequence,
                shouldSuspendTargetApp: true,
                enableStartupHook: true);
        ...
        public Task CapturesParametersAndOutputJsonSequence(Architecture targetArchitecture) =>
                CapturesParametersCore(
                    TestAppScenarios.ParameterCapturing.SubScenarios.AspNetApp,
                    targetArchitecture,
                    CapturedParameterFormat.JsonSequence,
                    shouldSuspendTargetApp: true,
                    enableStartupHook: true);
        ...
        public Task CapturesParametersAndOutputNewlineDelimitedJson(Architecture targetArchitecture) =>
                CapturesParametersCore(
                    TestAppScenarios.ParameterCapturing.SubScenarios.AspNetApp,
                    targetArchitecture, CapturedParameterFormat.NewlineDelimitedJson,
                    shouldSuspendTargetApp: true,
                    enableStartupHook: true);
        ...
        public Task CapturesParametersNoSuspend(Architecture targetArchitecture) =>
                CapturesParametersCore(
                    ...
                    targetArchitecture,
                    CapturedParameterFormat.JsonSequence,
                    shouldSuspendTargetApp: false,
                    enableStartupHook: false);
#endif // NET8_0_OR_GREATER

        [Theory]
        [MemberData(nameof(ProfilerHelper.GetArchitecture), MemberType = typeof(ProfilerHelper))]
        public async Task CapturesParametersNoSuspendDoesntWorkWithManuallyEnabledStartupHook(Architecture targetArchitecture)
        {
            await RunTestCaseCore(
                TestAppScenarios.ParameterCapturing.SubScenarios.AspNetApp,
                targetArchitecture,
                ...
                async (appRunner, apiClient) =>
                {
                    ...
                    OperationResponse response = await apiClient.CaptureParametersAsync(processId, CommonTestTimeouts.GeneralTimeout, config, CapturedParameterFormat.JsonSequence, FileProviderName);
                    ...
                    Assert.Equal(((int)HttpStatusCode.BadRequest).ToString(), operationResult.OperationStatus.Error.Code);
                });
        }
